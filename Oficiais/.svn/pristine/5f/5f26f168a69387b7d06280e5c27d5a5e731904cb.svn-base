Ext.define('Oficiais.util.JsonPUtil', {
...
// producao
    ...
    recuperaCredenciais : function(callBack) {
        Oficiais.util.DataBaseUtils.getValorParametro('CH_CR', function(valor){
            if(!Ext.isEmpty(valor)) {
                var tms = Ext.Date.now();
                var crd = '{lgn='+Oficiais.util.JsonPUtil.usuario+',sn='+Oficiais.util.JsonPUtil.senha+',tm=' + tms + '}';
                var key = CryptoJS.enc.Latin1.parse(valor);
                var iv  = CryptoJS.enc.Latin1.parse('a chave vem aqui');
                var padMsg = Oficiais.util.JsonPUtil.padString(crd);
    ...
    recuperaNovoToken : function() {
        ...
        var dtAnterior = new Date(Oficiais.util.JsonPUtil.timestamp);
        var diff = dtAgora.getTime() - dtAnterior.getTime();
        var diffMin = diff /(1000*60);
        if (Ext.isEmpty(Oficiais.util.JsonPUtil.token) || diffMin > Oficiais.util.JsonPUtil.qtdMinutos) {
            return true;
        }
        return false;
    },
    ...
    validaAutenticacao : function(cbSuccess, cbError) {
        var info = null;
        if(Oficiais.util.JsonPUtil.recuperaNovoToken()) {
            if(Ext.device.Connection.isOnline()) {
                Oficiais.util.JsonPUtil.autenticaUsuario(Oficiais.util.JsonPUtil.usuario, Oficiais.util.JsonPUtil.senha, 
    ...
    autenticaUsuario : function(usuario, senha, controller) {
    	Oficiais.util.JsonPUtil.usuario = usuario;
    	Oficiais.util.JsonPUtil.senha = senha;
        var objUsuario = {lgn: usuario, sn: senha};
        
        var urlPublicaFormulario = Oficiais.util.JsonPUtil.urlBaseServicos + '/usuario/userByLogin';
        Ext.Ajax.request({
            ...
            success: function(result) {
                ...
                if(!Ext.isEmpty(info)) {
                    if(info.codigo == 0) {
                        Oficiais.util.JsonPUtil.token = info.token;
                        Oficiais.util.JsonPUtil.timestamp = Ext.Date.now();
                    }
                }
    ...
    permissoesUsuario : function(login, objCallback) {
        ...
        Oficiais.util.JsonPUtil.validaAutenticacao(function (msg) {
            var urlAutenticacao = Oficiais.util.JsonPUtil.urlBaseServicos  + '/usuario/permisoesByLogin';
            var request = {token: Oficiais.util.JsonPUtil.token, lgn: login};
            Ext.Ajax.request({
                url: urlAutenticacao,
                headers: { 'Content-Type': 'application/json' },
                method: 'POST',
        ...
        }, function(msg){Oficiais.util.DeviceUtil.showAlertaConfirma('Erro', 'Erro ao recuperar formulários', function(){});});
    },
